...
export const DiagramBox = ({ cx, y, minWidth, height, title, line1 = "", line2 = "", line3 = "" }: Props) => {
  ...
  return (
    <g>
      ...
      {line1 && <text x={cx} y={y + 42} textAnchor="middle" fontSize={9}>{line1}</text>}
      {line2 && <text x={cx} y={y + 58} textAnchor="middle" fontSize={8}>{line2}</text>}
      {line3 && <text x={cx} y={y + 74} textAnchor="middle" fontSize={8}>{line3}</text>}
    </g>
  );
};
