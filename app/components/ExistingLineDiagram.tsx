...
import { formatCableLabel } from "../lib/calculations";
...
export const ExistingLineDiagram = ({ info, circuits, pageNum, totalPages }: Props) => {
  ...
  const mainBreakerLabel = info.mainBreakerSize === "אחר" ? info.mainBreakerCustomSize : info.mainBreakerSize;
  ...
  circuits.forEach((c, i) => {
    if (!c.isNew) {
      ...
      const label = `מעגל ${ec.circuitNumber || i + 1} (${ec.existingBreakerSize === "אחר" ? ((ec as any).existingBreakerCustomSize || "אחר") : ec.existingBreakerSize})`;
      ...
      summaryItems.push({ text: `FC${newIdx} — מפסק חדש ${nc.breaker === "אחר" ? ((nc as any).breakerCustomSize || "אחר") : nc.breaker}${nc.calibration ? ` | כיול ל-${nc.calibration}` : ""}`, isNew: true });
    ...
    <div className="pdf-page" style={{
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2px 8px", padding: "6px 10px", fontSize: 11, alignContent: "center" }}>
          ...
          <div><span style={{ color: "#666", fontSize: 10 }}>מפסק: </span><strong style={{ color: B }}>{mainBreakerLabel || "-"}</strong></div>
      ...
      <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
        ...
        <svg width={1100} height={svgH} viewBox={`0 0 1100 ${svgH}`} style={{ fontFamily: FONT }}>
          ...
          <DiagramBox cx={550} y={0} minWidth={165} height={90}
            ...
            line2={mainBreakerLabel || "לא צוין"}
            line3={calcIncLabel(mainBreakerLabel)} />
            ...
            if (!circuit.isNew) {
              ...
              return (
                <g key={circuit.id}>
                  ...
                  <DiagramBox cx={cx} y={BREAKER_TOP_Y} minWidth={105} height={BREAKER_H}
                    ...
                    line2={c.existingBreakerSize === "אחר" ? ((c as any).existingBreakerCustomSize || "אחר") : c.existingBreakerSize} />
                  ...
                  <text x={cx} y={BOTTOM_LINE_Y + 12} textAnchor="middle" fontSize={9} fill="#444">{formatCableLabel(c)}</text>
              ...
              return (
                <g key={circuit.id}>
                  ...
                  <text x={cx} y={BREAKER_TOP_Y + 34} textAnchor="middle" fontSize={9} fill={NC}>{`${c.circuitName || `FC${newIdx}`} | ${c.breaker === "אחר" ? ((c as any).breakerCustomSize || "אחר") : c.breaker}`}</text>
                  <text x={cx} y={BREAKER_TOP_Y + 48} textAnchor="middle" fontSize={8} fill={NC}>{calcIncLabel(c.breaker === "אחר" ? ((c as any).breakerCustomSize || "") : c.breaker)}</text>
                  ...
                  <text x={cx} y={BOTTOM_LINE_Y + 12} textAnchor="middle" fontSize={9} fill={NC}>{formatCableLabel(c)}</text>
