...
const getItems = (circuits: PanelCircuit[]): Item[] => {
  ...
  circuits.forEach((c, idx) => {
    ...
    if (!c.isNew) {
      ...
      const ecBreakerLabel = ec.existingBreakerSize === "אחר" ? ((ec as any).existingBreakerCustomSize || "אחר") : ec.existingBreakerSize;
      if (ec.calibration) items.push({ item: `כיול ${ec.existingBreakerType} ${ecBreakerLabel}`, qty: 1, notes: `${label} — כיול ל-${ec.calibration}`, isNew: false });
      if (ec.feedConnection === "ישירות למפסק - נעל כבל") {
        const cores = parseInt(ec.cableCores);
        items.push({ item: `נעל כבל ${lugMat} ${ec.cableSize} ממ"ר`, qty: cores, notes: `חיבור כבל הזנה ${formatCableLabel(ec)} — ${label}`, isNew: true });
      ...
      const ncBreakerLabel = nc.breaker === "אחר" ? ((nc as any).breakerCustomSize || "אחר") : nc.breaker;
      items.push({ item: `${nc.breakerType} ${ncBreakerLabel}`, qty: 1, notes: `${label} — מפסק חדש`, isNew: true });
      ...
      if (nc.calibration) items.push({ item: `כיול ${nc.breakerType} ${ncBreakerLabel}`, qty: 1, notes: `${label} — כיול ל-${nc.calibration}`, isNew: true });
      if (nc.feedConnection === "ישירות למפסק - נעל כבל") {
        const cores = parseInt(nc.cableCores);
        items.push({ item: `נעל כבל ${lugMat} ${nc.cableSize} ממ"ר`, qty: cores, notes: `חיבור כבל הזנה ${formatCableLabel(nc)} — ${label}`, isNew: true });
        if (nc.peCableSize !== "ללא") items.push({ item: `נעל כבל נחושת ${nc.peCableSize} ממ"ר`, qty: 1, notes: `גיד PE — ${label}`, isNew: true });
      }
      items.push({
        item: nc.internalFeedType === "פס נחושת גמישה"
          ? `פס נחושת גמישה למפסק ${ncBreakerLabel}`
          : `גיד גמיש PVC נחושת ${nc.internalFeedSize || nc.wireSize} ממ"ר`,
        qty: 1,
        notes: `${label} — הזנה פנימית למפסק`,
        isNew: true,
      });
    }
  });
