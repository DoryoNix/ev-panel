...
export const formatCableLabel = (c: { cableType: string; cableCores: string; cableSize: string; peCableSize?: string }): string => {
  const baseType = c.cableType.includes("NA2XY") ? "NA2XY" : c.cableType.includes("N2XY") ? "N2XY" : c.cableType;
  const base = `${c.cableCores}x${c.cableSize} ${baseType}`;
  if (c.cableCores === "4" && c.peCableSize && c.peCableSize !== "ללא") {
    return `${base} + ${c.peCableSize} CU PVC`;
  }
  return base;
};
